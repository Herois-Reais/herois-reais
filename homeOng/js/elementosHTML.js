const btnMenuHamburguer = document.getElementById('btnMenuH');
const perfilOngContainer = document.getElementById('perfilOngC');
const iconeMenuHam = document.querySelector('.fa-bars')
const iconeMenuX = document.querySelector('.fa-x')
const sectionMenuHamX = document.querySelector('.areaMenuHamburguer')
const btnExcluirPerfilOng = document.getElementById('btnExcluirPerfil')

const imgPerfilOng = document.getElementById('imageUpload')
const imgPreview = document.getElementById('imgSubida')
const textUpload = document.querySelector('.textUpload')

const telefoneOng = document.getElementById('inputTelefone')
const nomeOng = document.getElementById('inputNomeOng')
const senhaOng = document.getElementById('inputSenha')
const cnpjOng = document.getElementById('inputCnpjOng')
const botaoSalvarAlteracoes = document.getElementById('btnSalvarAlteracoes')
const emailOng = document.getElementById('inputEmail')

export {
    btnMenuHamburguer,
    perfilOngContainer,
    iconeMenuHam,
    iconeMenuX,
    sectionMenuHamX,
    btnExcluirPerfilOng,
    imgPerfilOng,
    imgPreview,
    textUpload,
    telefoneOng,
    nomeOng,
    senhaOng,
    cnpjOng,
    botaoSalvarAlteracoes,
    emailOng
}